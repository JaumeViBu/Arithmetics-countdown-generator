...
function genSeedNumbers() {
  ...
  seeds.push(genSeedCandidate(1, 11));
  seeds.push(genSeedCandidate(1, 11));
  seeds.push(genSeedCandidate(11, 51));
  seeds.push(genSeedCandidate(11, 51));
  seeds.push(genSeedCandidate(51, 101));
  seeds.push(genSeedCandidate(51, 101));
  seeds.push(genSeedCandidate(101, 301));
  seeds.push(genSeedCandidate(101, 401));
 ...
 *  
 * @param {Array<number>} seedsNumbers A seedNumbers array, better generated using genSeedNumbers function
...
function genTargetNumbers(seedsNumbers) {
  ...
  for (let i = 0; res.length < TARGET_NUMBERS; i++) {
    ...
    for (let j = 0; j < random_seeds; j++) {
      ...
      candidates.push(tempSeeds.splice(randomSeedIndex, 1));
    }

    let success = true;
    do {

      let a = candidates.splice(Math.floor(random(0, candidates.length)));
      let b = candidates.splice(Math.floor(random(0, candidates.length)));
...
for (let i = 0; i < 200; i++) {
  ...
  initiateLists();
  printLog(seedsNumbers, targetNumbers);
}
